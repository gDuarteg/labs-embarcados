...
/** Wi-Fi Settings */
#define MAIN_WLAN_SSID    "Ocaimbra" /**< Destination SSID */
#define MAIN_WLAN_PSK     "senha123" /**< Password for Destination SSID */

#define MAIN_SERVER_PORT  5000
#define MAIN_SERVER_NAME  "192.168.0.51"
...
void send_request(char buffer[], char message[]) {
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
      ...
      case GET:
      printf("STATE: GET \n");
	  send_request(g_sendBuffer, "/status");
      state = ACK;
      break;
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        printf(STRING_LINE);
        printf(p_recvMsg->pu8Buffer);
		char* teste = strstr(p_recvMsg->pu8Buffer, "led");
		printf("ACHOU: %c\n", teste[7]);
		if(teste[7] == '1'){
